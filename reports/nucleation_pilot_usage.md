...
If Phase B leaves the ligand-rich edge incomplete, run `scripts/nucleation_k7_p1_p10_completion_24c.slurm`. It imports the Phase-B archive, selects only p8--p10 parents, evaluates only p9/p10 fixed-k endpoints, and has an 800-call ceiling. A phase cannot report convergence until every configured composition has its requested number of independent primary families; the CdSe k7 target is at least five p9 and three p10 families. Missing-bin debt is written into the phase halt record.

### Phase C: bounded k=7 to k=8 extension

Submit `scripts/nucleation_phase_c_k8_24c.slurm` with `nucleation_phase_b_p1_p10_completion_v1` as its source. The 160-family k7 parent cohort is composition-stratified and explicitly includes three primary families from every p1--p10 bin. The k8 beam has 180 slots with a 15% admission reserve, independent 3,300-call growth and 3,700-call fixed-k caps, and a three-family coverage requirement for every p1--p11 composition. Phase C uses the 1 eV energy-window plateau and cannot extend beyond k8.
